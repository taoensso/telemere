(ns taoensso.telemere-tests
  (:require
   [clojure.test                :as test :refer [deftest testing is]]
   [taoensso.encore             :as enc  :refer [throws? submap?]]
   [taoensso.encore.signals.api :as sigs-api]
   [taoensso.telemere           :as tel]
   [taoensso.telemere.impl      :as impl]))

(comment
  (remove-ns      'taoensso.telemere-tests)
  (test/run-tests 'taoensso.telemere-tests))

;;;; Aliases

(enc/defaliases
  {:alias ws   :src impl/with-signal}
  {:alias sig! :src impl/signal!})

;;;; Utils

(defn- throw! [] (throw (ex-info "TestEx" {})))

;;;;

;; Note lots of low-level signal/filtering tests in `taoensso.encore`

(deftest _parse-msg-form
  (let [msg @#'impl/parse-msg-form]
    [(is (= (msg '["foo"]))      '"foo")
     (is (= (msg '["foo" "bar"]) '(clojure.core/delay (taoensso.telemere.impl/signal-msg ["foo" "bar"]))))
     (is (= (msg 'my-symbol)     'my-symbol))]))

(deftest _impl-misc
  [(is (= (impl/signal-msg
            ["x" "y" nil ["z1" nil "z2" "z3"]
             (impl/msg-splice ["s1" nil "s2" "s3" (impl/msg-skip) "s4"])
             (impl/msg-splice nil)
             (impl/msg-skip) :kw])

         "xynil[\"z1\" nil \"z2\" \"z3\"]s1nils2s3s4:kw"))])

(deftest _signal-macro
  [(is (=   (ws (sig! {:level :info, :elide? true               })) [nil nil]) "With compile-time elision")
   (is (=   (ws (sig! {:level :info, :elide? true,  :run (+ 1 2)})) [3   nil]) "With compile-time elision, run-form")
   (is (=   (ws (sig! {:level :info, :allow? false              })) [nil nil]) "With runtime suppression")
   (is (=   (ws (sig! {:level :info, :allow? false, :run (+ 1 2)})) [3   nil]) "With runtime suppression, run-form")

   (is (->>     (sig! {:level :info, :elide? true,  :run (throw!)}) (throws? :ex-info "TestEx")) "With compile-time elision, throwing run-form")
   (is (->>     (sig! {:level :info, :allow? false, :run (throw!)}) (throws? :ex-info "TestEx")) "With runtime suppression,  throwing run-form")

   (let [[rv1 sig1] (ws (sig! {:level :info              }))
         [rv2 sig2] (ws (sig! {:level :info, :run (+ 1 2)}))]

     [(is (= rv1 nil)) (is (submap? sig1 {:ns "taoensso.telemere-tests", :level :info, :run-form nil,      :run-value nil, :runtime-nsecs 0}))
      (is (= rv2   3)) (is (submap? sig2 {:ns "taoensso.telemere-tests", :level :info, :run-form '(+ 1 2), :run-value 3,   :runtime-nsecs (enc/pred pos?)}))])

   (testing "Nested signals"
     (let [[[inner-rv inner-sig] outer-sig] (ws (sig! {:level :info, :run (ws (sig! {:level :warn, :run "inner-run"}))}))]
       [(is (= inner-rv "inner-run"))
        (is (enc/submap? inner-sig {:level :warn, :run-value "inner-run"}))
        (is (enc/submap? outer-sig {:level :info  :run-value [inner-rv inner-sig]}))]))

   (testing "Timestamps"
     (let [[_ sig1] (ws (sig! {:level :info                             }))
           [_ sig2] (ws (sig! {:level :info, :run (reduce + (range 1e6))}))
           [_ sig3] (ws (sig! {:level :info, :run (reduce + (range 1e6))
                               :timestamp ; Allow custom timestamp
                               #?(:clj  java.time.Instant/EPOCH
                                  :cljs (js/Date. 0))}))]

       [(let [{start :timestamp, end :end-timestamp} sig1]
          [(is (enc/inst? start))
           (is (enc/inst? end))
           (is   (= start end))])

        (let [{start :timestamp, end :end-timestamp} sig2]
          [(is (enc/inst?  start))
           (is (enc/inst?  end))
           (is (> (inst-ms end) (inst-ms start)))])

        (let [{start :timestamp, end :end-timestamp} sig3]
          [(is (enc/inst?  start))
           (is (enc/inst?  end))
           (is (= (inst-ms start) 0)               "Respect custom timestamp")
           (is (> (inst-ms end)   (inst-ms start)) "End timestamp is start + runtime-nsecs")
           (is (< (inst-ms end)   1e6)             "End timestamp is start + runtime-nsecs")])]))

   (testing "User opts assoced directly to signal"
     (let [[rv sig] (ws (sig! {:level :info, :my-opt1 "v1", :my-opt2 "v2"}))]
       (is (submap?      sig  {:level :info, :my-opt1 "v1", :my-opt2 "v2"}))))

   (testing "`:msg` basics"
     (let [c          (enc/counter)
           [rv1 sig1] (ws (sig! {:level :info, :run (c), :msg      "msg1"}))
           [rv2 sig2] (ws (sig! {:level :info, :run (c), :msg [    "msg2:" (c)]}))
           [rv3 sig3] (ws (sig! {:level :info, :run (c), :msg (str "msg3:" (c))}))
           [rv4 sig4] (ws (sig! {:level :info, :run (c), :msg (str "msg4:" (c)), :allow? false}))]

       [(is (= rv1 0)) (is (=        (:msg_ sig1)  "msg1"))
        (is (= rv2 1)) (is (= (force (:msg_ sig2)) "msg2:4"))
        (is (= rv3 2)) (is (= (force (:msg_ sig3)) "msg3:5"))
        (is (= rv4 3)) (is (= (force (:msg_ sig4)) nil))
        (is (= @c  6)  "4x run + 2x message (1x suppressed)")]))

   (testing "`:data` basics"
     (vec
       (for [dk [:data :my-opt]] ; User opts share same behaviour as data
         (let [c          (enc/counter)
               [rv1 sig1] (ws (sig! {:level :info, :run (c), dk        {:c1 (c)}}))
               [rv2 sig2] (ws (sig! {:level :info, :run (c), dk (delay {:c2 (c)})}))
               [rv3 sig3] (ws (sig! {:level :info, :run (c), dk        {:c3 (c)},  :allow? false}))
               [rv4 sig4] (ws (sig! {:level :info, :run (c), dk (delay {:c4 (c)}), :allow? false}))
               [rv5 sig5] (ws (sig! {:level :info, :run (c), dk        [:c5 (c)]}))
               [rv6 sig6] (ws (sig! {:level :info, :run (c), dk (delay [:c6 (c)])}))]

           [(is (= rv1 0)) (is (=        (get sig1 dk)  {:c1 1}))
            (is (= rv2 2)) (is (= (force (get sig2 dk)) {:c2 8}))
            (is (= rv3 3)) (is (=        (get sig3 dk)  nil))
            (is (= rv4 4)) (is (= (force (get sig4 dk)) nil))
            (is (= rv5 5)) (is (=        (get sig5 dk)  [:c5 6]) "`:data` can be any type")
            (is (= rv6 7)) (is (= (force (get sig6 dk)) [:c6 9]) "`:data` can be any type")
            (is (= @c  10) "6x run + 4x data (2x suppressed)")]))))

   (testing "`:let` basics"
     (let [c          (enc/counter)
           [rv1 sig1] (ws (sig! {:level :info, :run (c), :let [_ (c)]}))
           [rv2 sig2] (ws (sig! {:level :info, :run (c), :let [_ (c)], :allow? false}))
           [rv3 sig3] (ws (sig! {:level :info, :run (c), :let [_ (c)]}))]

       [(is (= rv1 0))
        (is (= rv2 2))
        (is (= rv3 3))
        (is (= @c  5) "3x run + 2x let (1x suppressed)")]))

   (testing "`:let` + `:msg`"
     (let [c          (enc/counter)
           [rv1 sig1] (ws (sig! {:level :info, :run (c), :let [n (c)], :msg      "msg1"}))
           [rv2 sig2] (ws (sig! {:level :info, :run (c), :let [n (c)], :msg [    "msg2:" n ":" (c)]}))
           [rv3 sig3] (ws (sig! {:level :info, :run (c), :let [n (c)], :msg (str "msg3:" n ":" (c))}))
           [rv4 sig4] (ws (sig! {:level :info, :run (c), :let [n (c)], :msg (str "msg4:" n ":" (c)), :allow? false}))]

       [(is (= rv1 0)) (is (=        (:msg_ sig1)  "msg1"))
        (is (= rv2 2)) (is (= (force (:msg_ sig2)) "msg2:3:7"))
        (is (= rv3 4)) (is (= (force (:msg_ sig3)) "msg3:5:8"))
        (is (= rv4 6)) (is (= (force (:msg_ sig4)) nil))
        (is (= @c  9)  "4x run + 3x let (1x suppressed) + 3x msg (1x suppressed)")]))

   (testing "`:let` + `:data`"
     (vec
       (for [dk [:data :my-opt]]
         (let [c          (enc/counter)
               [rv1 sig1] (ws (sig! {:level :info, :run (c), :let [n (c)], dk        {:n n, :c1 (c)}}))
               [rv2 sig2] (ws (sig! {:level :info, :run (c), :let [n (c)], dk (delay {:n n, :c2 (c)})}))
               [rv3 sig3] (ws (sig! {:level :info, :run (c), :let [n (c)], dk        {:n n, :c3 (c)},  :allow? false}))
               [rv4 sig4] (ws (sig! {:level :info, :run (c), :let [n (c)], dk (delay {:n n, :c4 (c)}), :allow? false}))
               [rv5 sig5] (ws (sig! {:level :info, :run (c), :let [n (c)], dk        [:n n, :c5 (c)]}))
               [rv6 sig6] (ws (sig! {:level :info, :run (c), :let [n (c)], dk (delay [:n n, :c6 (c)])}))]

           [(is (= rv1 0))  (is (=        (get sig1 dk)  {:n 1, :c1 2}))
            (is (= rv2 3))  (is (= (force (get sig2 dk)) {:n 4, :c2 12}))
            (is (= rv3 5))  (is (=        (get sig3 dk)  nil))
            (is (= rv4 6))  (is (= (force (get sig4 dk)) nil))
            (is (= rv5 7))  (is (=        (get sig5 dk)  [:n 8,  :c5 9]))
            (is (= rv6 10)) (is (= (force (get sig6 dk)) [:n 11, :c6 13]))
            (is (= @c  14)  "6x run + 4x let (2x suppressed) + 4x data (2x suppressed)")]))))

   (testing "Manual `let` (unconditional) + `:data`"
     (vec
       (for [dk [:data #_:my-opt]]
         (let [c          (enc/counter)
               [rv1 sig1] (ws (let [n (c)] (sig! {:level :info, :run (c), dk        {:n n, :c1 (c)}})))
               [rv2 sig2] (ws (let [n (c)] (sig! {:level :info, :run (c), dk (delay {:n n, :c2 (c)})})))
               [rv3 sig3] (ws (let [n (c)] (sig! {:level :info, :run (c), dk        {:n n, :c3 (c)},  :allow? false})))
               [rv4 sig4] (ws (let [n (c)] (sig! {:level :info, :run (c), dk (delay {:n n, :c4 (c)}), :allow? false})))
               [rv5 sig5] (ws (let [n (c)] (sig! {:level :info, :run (c), dk        [:n n, :c5 (c)]})))
               [rv6 sig6] (ws (let [n (c)] (sig! {:level :info, :run (c), dk (delay [:n n, :c6 (c)])})))]

           [
            (is (= rv1 1))  (is (=        (get sig1 dk)  {:n 0, :c1 2}))
            (is (= rv2 4))  (is (= (force (get sig2 dk)) {:n 3, :c2 14}))
            (is (= rv3 6))  (is (=        (get sig3 dk)  nil))
            (is (= rv4 8))  (is (= (force (get sig4 dk)) nil))
            (is (= rv5 10)) (is (=        (get sig5 dk)  [:n 9,  :c5 11]))
            (is (= rv6 13)) (is (= (force (get sig6 dk)) [:n 12, :c6 15]))
            (is (= @c  16)  "6x run + 6x let (0x suppressed) + 4x data (2x suppressed)")]))))

   (testing "Call middleware"
     (let [c         (enc/counter)
           [rv1 sv1] (ws (sig! {:level :info, :run (c), :middleware [#(assoc % :m1 (c)) #(assoc % :m2 (c))]}))
           [rv2 sv2] (ws (sig! {:level :info, :run (c), :middleware [#(assoc % :m1 (c)) #(assoc % :m2 (c))], :allow? false}))
           [rv3 sv3] (ws (sig! {:level :info, :run (c), :middleware [#(assoc % :m1 (c)) #(assoc % :m2 (c))]}))
           [rv4 sv4] (ws (sig! {:level :info,           :middleware [(fn [_] "signal-value")]}))]

       [(is (= rv1 0))   (is (submap? sv1 {:m1 1 :m2 2}))
        (is (= rv2 3))   (is (nil?    sv2))
        (is (= rv3 4))   (is (submap? sv3 {:m1 5 :m2 6}))
        (is (= rv4 nil)) (is (=       sv4 "signal-value"))
        (is (= @c  7)  "3x run + 4x middleware")]))])

(deftest _handlers
  ;; Basic handler tests are in Encore
  [(testing "Handler middleware"
     (let [c      (enc/counter)
           sv-h1_ (atom nil)
           sv-h2_ (atom nil)
           wh1    (sigs-api/wrap-handler :hid1 (fn [sv] (reset! sv-h1_ sv)) {:async nil, :middleware [#(assoc % :hm1 (c)) #(assoc % :hm2 (c))]})
           wh2    (sigs-api/wrap-handler :hid2 (fn [sv] (reset! sv-h2_ sv)) {:async nil, :middleware [#(assoc % :hm1 (c)) #(assoc % :hm2 (c))]})]

       ;; Note that call middleware output is cached and shared across all handlers
       (binding [impl/*sig-handlers* {:hid1 wh1, :hid2 wh2}]
         (let [;; 1x run + 4x handler middleware + 2x call middleware = 7x
               rv1    (sig! {:level :info, :run (c), :middleware [#(assoc % :m1 (c)) #(assoc % :m2 (c))]})
               sv1-h1 @sv-h1_
               sv1-h2 @sv-h2_
               c1     @c

               ;; 1x run
               rv2    (sig! {:level :info, :run (c), :middleware [#(assoc % :m1 (c)) #(assoc % :m2 (c))], :allow? false})
               sv2-h1 @sv-h1_
               sv2-h2 @sv-h2_
               c2     @c ; 8

               ;; 1x run + 4x handler middleware + 2x call middleware = 7x
               rv3    (sig! {:level :info, :run (c), :middleware [#(assoc % :m1 (c)) #(assoc % :m2 (c))]})
               sv3-h1 @sv-h1_
               sv3-h2 @sv-h2_
               c3     @c ; 15

               ;; 4x handler middleware
               rv4    (sig! {:level :info,           :middleware [(fn [_] {:my-sig-val? true})]})
               sv4-h1 @sv-h1_
               sv4-h2 @sv-h2_
               c4     @c]

           [(is (= rv1 0))   (is (submap? sv1-h1 {:m1 1, :m2 2,      :hm1 3,  :hm2 4}))  (is (submap? sv1-h2 {:m1 1, :m2 2,      :hm1 5,  :hm2 6}))
            (is (= rv2 7))   (is (submap? sv2-h1 {:m1 1, :m2 2,      :hm1 3,  :hm2 4}))  (is (submap? sv2-h2 {:m1 1, :m2 2,      :hm1 5,  :hm2 6}))
            (is (= rv3 8))   (is (submap? sv3-h1 {:m1 9, :m2 10,     :hm1 11, :hm2 12})) (is (submap? sv3-h2 {:m1 9, :m2 10,     :hm1 13, :hm2 14}))
            (is (= rv4 nil)) (is (submap? sv4-h1 {:my-sig-val? true, :hm1 15, :hm2 16})) (is (submap? sv4-h2 {:my-sig-val? true, :hm1 17, :hm2 18}))
            (is (= c1  7)    "1x run +  4x handler middleware + 2x call middleware")
            (is (= c2  8)    "2x run +  4x handler middleware + 2x call middleware")
            (is (= c3  15)   "3x run +  8x handler middleware + 4x call middleware")
            (is (= c4  19)   "3x run + 12x handler middleware + 4x call middleware")]))))])

(def ^:private ^:dynamic *throwing-handler-middleware?* false)

(deftest _throwing
  (let [sv_    (atom :nx)
        error_ (atom :nx)
        reset-state!
        (fn []
          (reset! sv_    :nx)
          (reset! error_ :nx)
          true)]

    (sigs-api/with-handler impl/*sig-handlers* :hid1
      (fn [sv] (force (:data sv)) (reset! sv_ sv))
      {:async nil, :error-fn (fn [x] (reset! error_ x)), :rl-error nil,
       :middleware [(fn [sv] (if *throwing-handler-middleware?* (throw!) sv))]}

      [(is (->>    (sig! {:level :info, :filter    (throw!)}) (throws? :ex-info "TestEx")) "`~filterable-expansion/allow` throws at call")
       (is (->>    (sig! {:level :info, :timestamp (throw!)}) (throws? :ex-info "TestEx")) "`~timestamp-form`             throws at call")
       (is (->>    (sig! {:level :info, :id        (throw!)}) (throws? :ex-info "TestEx")) "`~id-form`                    throws at call")
       (is (->>    (sig! {:level :info, :uid       (throw!)}) (throws? :ex-info "TestEx")) "`~uid-form`                   throws at call")
       (is (->>    (sig! {:level :info, :run       (throw!)}) (throws? :ex-info "TestEx")) "`~run-form` rethrows at call")
       (is (submap? @sv_ {:level :info, :error (enc/pred enc/error?)})                     "`~run-form` rethrows at call *after* dispatch")

       (testing "`@signal-value_`: trap with wrapped handler"
         [(testing "Throwing `~let-form`"
            (reset-state!)
            [(is (nil? (sig! {:level :info, :let [_ (throw!)]})))
             (is (= @sv_ :nx))
             (is (submap? @error_ {:handler-id :hid1, :error (enc/pred enc/error?)}))])

          (testing "Throwing call middleware"
            (reset-state!)
            [(is (nil? (sig! {:level :info, :middleware [(fn [_] (throw!))]})))
             (is (= @sv_ :nx))
             (is (submap? @error_ {:handler-id :hid1, :error (enc/pred enc/error?)}))])

          (testing "Throwing handler middleware"
            (reset-state!)
            (binding [*throwing-handler-middleware?* true]
              [(is (nil? (sig! {:level :info})))
               (is (= @sv_ :nx))
               (is (submap? @error_ {:handler-id :hid1, :error (enc/pred enc/error?)}))]))

          (testing "Throwing `@data_`"
            (reset-state!)
            [(is (nil? (sig! {:level :info, :data (delay (throw!))})))
             (is (= @sv_ :nx))
             (is (submap? @error_ {:handler-id :hid1, :error (enc/pred enc/error?)}))])

          (testing "Throwing user opt"
            (reset-state!)
            [(is (nil? (sig! {:level :info, :my-opt (throw!)})))
             (is (= @sv_ :nx))
             (is (submap? @error_ {:handler-id :hid1, :error (enc/pred enc/error?)}))])])])))

(deftest _ctx
  (testing "Context (`*ctx*`)"
    [(is (= (binding [tel/*ctx* "my-ctx"] tel/*ctx*) "my-ctx") "Supports manual `binding`")
     (is (= (tel/with-ctx       "my-ctx"  tel/*ctx*) "my-ctx") "Supports any data type")

     (is (= (tel/with-ctx "my-ctx1"       (tel/with-ctx+ nil                        tel/*ctx*)) "my-ctx1")              "nil update => keep old-ctx")
     (is (= (tel/with-ctx "my-ctx1"       (tel/with-ctx+ (fn [old] [old "my-ctx2"]) tel/*ctx*)) ["my-ctx1" "my-ctx2"])  "fn  update => apply")
     (is (= (tel/with-ctx {:a :A1 :b :B1} (tel/with-ctx+ {:a :A2 :c :C2}            tel/*ctx*)) {:a :A2 :b :B1 :c :C2}) "map update => merge")

     (let [[_ sig] (ws (sig! {:level :info, :ctx "my-ctx"}))] (is (enc/submap? sig {:ctx "my-ctx"}) "Can be set via call opt"))]))

(deftest _tracing
  (testing "Tracing"
    [(let [[_ sig] (ws (sig! {:level :info                      }))] (is (enc/submap? sig {:parent nil})))
     (let [[_ sig] (ws (sig! {:level :info, :parent {:id   :id0}}))] (is (enc/submap? sig {:parent {:id :id0       :uid :submap/nx}}) "`:parent/id`  can be set via call opt"))
     (let [[_ sig] (ws (sig! {:level :info, :parent {:uid :uid0}}))] (is (enc/submap? sig {:parent {:id :submap/nx :uid      :uid0}}) "`:parent/uid` can be set via call opt"))

     (testing "Auto call id, uid"
       (let [[_ sig] (ws (sig! {:level :info, :parent {:id :id0, :uid :uid0}, :run impl/*trace-parent*, :data impl/*trace-parent*}))]
         [(is (enc/submap? sig {:parent    {:id :id0, :uid :uid0}}))
          (is (enc/submap? sig {:run-value {:id nil,  :uid (get sig :uid ::nx)}}) "`*trace-parent*`     visible to run-form, bound to call's auto {:keys [id uid]}")
          (is (enc/submap? sig {:data      nil})                                  "`*trace-parent*` not visible to data-form ")]))

     (testing "Manual call id, uid"
       (let [[_ sig] (ws (sig! {:level :info, :parent {:id :id0, :uid :uid0}, :id :id1, :uid :uid1, :run impl/*trace-parent*, :data impl/*trace-parent*}))]
         [(is (enc/submap? sig {:parent    {:id :id0, :uid :uid0}}))
          (is (enc/submap? sig {:run-value {:id :id1, :uid :uid1}}) "`*trace-parent*`     visible to run-form, bound to call's auto {:keys [id uid]}")
          (is (enc/submap? sig {:data      nil})                    "`*trace-parent*` not visible to data-form ")]))

     (testing "Tracing can be disabled via call opt"
       (let [[_ sig] (ws (sig! {:level :info, :parent {:id :id0, :uid :uid0}, :id :id1, :uid :uid1, :run impl/*trace-parent*, :data impl/*trace-parent*, :trace? false}))]
         [(is (enc/submap? sig {:parent    {:id :id0, :uid :uid0}}))
          (is (enc/submap? sig {:run-value nil}))]))

     (testing "Signal nesting"
       (let [[[inner-rv inner-sig] outer-sig]
             (ws (sig! {                :level :info, :id :id1, :uid :uid1,
                        :run (ws (sig! {:level :info, :id :id2, :uid :uid2, :run impl/*trace-parent*}))}))]

         [(is (enc/submap? outer-sig             {:id :id1, :uid :uid1, :parent nil}))
          (is (enc/submap? inner-rv              {:id :id2, :uid :uid2}))
          (is (enc/submap? inner-sig {:parent    {:id :id1, :uid :uid1}}))
          (is (enc/submap? inner-sig {:run-value {:id :id2, :uid :uid2}}))]))]))

;;;;

#?(:cljs (test/run-tests))
