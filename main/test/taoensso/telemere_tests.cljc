...
(deftest _common-signals
  [(testing "event!" ; id + ?level => allowed?
     [(let [{rv :value, [sv] :signals} (with-sigs (tel/event! :id1                    ))] [(is (= rv true)) (is (sm?  sv {:kind :event, :coords vector?, :level :info, :id :id1}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/event! :id1              :warn)) ] [(is (= rv true)) (is (sm?  sv {:kind :event, :coords vector?, :level :warn, :id :id1}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/event! :id1      {:level :warn}))] [(is (= rv true)) (is (sm?  sv {:kind :event, :coords vector?, :level :warn, :id :id1}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/event! {:id :id1, :level :warn}))] [(is (= rv true)) (is (sm?  sv {:kind :event, :coords vector?, :level :warn, :id :id1}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/event! :id1 {:allow? false}))    ] [(is (= rv nil))  (is (nil? sv))])])

   (testing "log!" ; ?level + msg => allowed?
     [(let [{rv :value, [sv] :signals} (with-sigs (tel/log!                      "msg")) ] [(is (= rv true)) (is (sm?  sv {:kind :log, :coords vector?, :msg_ "msg", :level :info}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/log!          :warn       "msg")) ] [(is (= rv true)) (is (sm?  sv {:kind :log, :coords vector?, :msg_ "msg", :level :warn}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/log! {:level  :warn}      "msg")) ] [(is (= rv true)) (is (sm?  sv {:kind :log, :coords vector?, :msg_ "msg", :level :warn}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/log! {:level  :warn, :msg "msg"}))] [(is (= rv true)) (is (sm?  sv {:kind :log, :coords vector?, :msg_ "msg", :level :warn}))])
   ...
   (testing "trace!" ; ?id + run => unconditional run result (value or throw)
     [(let [{rv :value, [sv] :signals} (with-sigs (tel/trace!                 (+ 1 2))) ] [(is (= rv 3))  (is (sm?  sv {:kind :trace, :coords vector?, :level :info, :id  nil, :msg_ "(+ 1 2) => 3"}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/trace! {:msg nil}      (+ 1 2))) ] [(is (= rv 3))  (is (sm?  sv {:kind :trace, :coords vector?, :level :info, :id  nil, :msg_ nil}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/trace!      :id1       (+ 1 2))) ] [(is (= rv 3))  (is (sm?  sv {:kind :trace, :coords vector?, :level :info, :id :id1}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/trace! {:id :id1}      (+ 1 2))) ] [(is (= rv 3))  (is (sm?  sv {:kind :trace, :coords vector?, :level :info, :id :id1}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/trace! {:id :id1, :run (+ 1 2)}))] [(is (= rv 3))  (is (sm?  sv {:kind :trace, :coords vector?, :level :info, :id :id1}))])
      (let [{re :error, [sv] :signals} (with-sigs (tel/trace!      :id1        (ex1!))) ] [(is (ex1? re)) (is (sm?  sv {:kind :trace, :coords vector?, :level :info, :id :id1, :error ex1,
      ...
      (testing "Use with `catch->error!`"
        (let [{re :error, [sv1 sv2] :signals} (with-sigs (tel/trace! ::id1 (tel/catch->error! ::id2 (ex1!))))]
          [(is (ex1? re))
           (is (sm? sv1 {:kind :error, :coords vector?, :level :error, :id ::id2, :error ex1, :parent {:id ::id1}}))
           (is (sm? sv2 {:kind :trace, :coords vector?, :level :info,  :id ::id1, :error ex1, :root   {:id ::id1}}))]))
   ...
   (testing "spy" ; ?level + run => unconditional run result (value or throw)
     [(let [{rv :value, [sv] :signals} (with-sigs (tel/spy!                     (+ 1 2))) ] [(is (= rv 3))  (is (sm?  sv {:kind :spy, :coords vector?, :level :info, :msg_ "(+ 1 2) => 3"}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/spy! {:msg nil}          (+ 1 2))) ] [(is (= rv 3))  (is (sm?  sv {:kind :spy, :coords vector?, :level :info, :msg_ nil}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/spy!         :warn       (+ 1 2))) ] [(is (= rv 3))  (is (sm?  sv {:kind :spy, :coords vector?, :level :warn}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/spy! {:level :warn}      (+ 1 2))) ] [(is (= rv 3))  (is (sm?  sv {:kind :spy, :coords vector?, :level :warn}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/spy! {:level :warn, :run (+ 1 2)}))] [(is (= rv 3))  (is (sm?  sv {:kind :spy, :coords vector?, :level :warn}))])
      (let [{re :error, [sv] :signals} (with-sigs (tel/spy!         :warn         (ex1!)))] [(is (ex1? re)) (is (sm?  sv {:kind :spy, :coords vector?, :level :warn, :error ex1,
                                                                                                             :msg_ #?(:clj  "(ex1!) !> clojure.lang.ExceptionInfo"
                                                                                                                      :cljs "(ex1!) !> cljs.core/ExceptionInfo")}))])
      ...
      (testing "Use with `catch->error!`"
        (let [{re :error, [sv1 sv2] :signals} (with-sigs (tel/spy! :warn (tel/catch->error! :error (ex1!))))]
          [(is (ex1? re))
           (is (sm? sv1 {:kind :error, :coords vector?, :level :error, :error ex1, :parent {}}))
           (is (sm? sv2 {:kind :spy,   :coords vector?, :level :warn,  :error ex1, :root   {}}))]))])

   (testing "error!" ; ?id + error => unconditional given error
     [(let [{rv :value, [sv] :signals} (with-sigs (tel/error!                        ex1)) ] [(is (ex1? rv)) (is (sm?  sv {:kind :error, :coords vector?, :level :error, :error ex1, :id  nil}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/error!           :id1         ex1)) ] [(is (ex1? rv)) (is (sm?  sv {:kind :error, :coords vector?, :level :error, :error ex1, :id :id1}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/error! {:id      :id1}        ex1)) ] [(is (ex1? rv)) (is (sm?  sv {:kind :error, :coords vector?, :level :error, :error ex1, :id :id1}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/error! {:id      :id1, :error ex1}))] [(is (ex1? rv)) (is (sm?  sv {:kind :error, :coords vector?, :level :error, :error ex1, :id :id1}))])
   ...
   (testing "catch->error!" ; ?id + run => unconditional run value or ?return
     [(let [{rv :value, [sv] :signals} (with-sigs (tel/catch->error!                   (+ 1 2)))] [(is (= rv    3)) (is (nil? sv))])
      (let [{re :error, [sv] :signals} (with-sigs (tel/catch->error!                    (ex1!)))] [(is (ex1?   re)) (is (sm? sv {:kind :error, :coords vector?, :level :error, :error ex1, :id  nil}))])
      (let [{re :error, [sv] :signals} (with-sigs (tel/catch->error!             :id1   (ex1!)))] [(is (ex1?   re)) (is (sm? sv {:kind :error, :coords vector?, :level :error, :error ex1, :id :id1}))])
      (let [{re :error, [sv] :signals} (with-sigs (tel/catch->error! {:id        :id1}  (ex1!)))] [(is (ex1?   re)) (is (sm? sv {:kind :error, :coords vector?, :level :error, :error ex1, :id :id1}))])
      (let [{rv :value, [sv] :signals} (with-sigs (tel/catch->error! {:catch-val :foo}  (ex1!)))] [(is (= rv :foo)) (is (sm? sv {:kind :error, :coords vector?, :level :error, :error ex1, :id  nil}))])
      ...
      (testing "uncaught->error!"
        (let [sv_ (atom ::nx)]
          [(do (enc/set-var-root! impl/*sig-handlers* [(sigs/wrap-handler "h1" (fn h1 [x] (reset! sv_ x)) nil {:async nil})]) :set-handler)
           ...
           (is (do (.join (enc/threaded :user (ex1!))) (sm? @sv_ {:kind :error, :coords vector?, :level :error, :error ex1, :id  nil})))
           ;;
           (is (nil? (tel/uncaught->error! :id1)))
           (is (do (.join (enc/threaded :user (ex1!))) (sm? @sv_ {:kind :error, :coords vector?, :level :error, :error ex1, :id :id1})))
           ;;
...
(deftest _dispatch-signal!
  [(sm? (tel/with-signal (tel/dispatch-signal! (assoc (tel/with-signal :trap (tel/log! "hello")) :level :warn)))
     {:kind :log, :level :warn, :coords vector?})])
