...
(def ^:dynamic *uid-fn*
  ...
  Relevant only when `otel-mode?` is false.
  If `otel-mode?` is true, uids are instead generated by OpenTelemetry.
...
#?(:clj
   (def otel-mode?
     ...
     Should Telemere macro expansions interop with OpenTelemetry Java [1]?

     When true:

       - Telemere's tracing signal creators (`trace!`, `spy!`, etc.) will
         manipulate OpenTelemetry's `Context` to add standard `Span` info
         when relevant. This'll enable interop with other manual/auto
         OpenTelemetry tools/instrumentation.

       - Telemere's OpenTelemetry handler will be automatically registered
         with default options. Re-configure or remove it like any other handler.
         See `taoensso.telemere.open-telemetry/handler:open-telemetry` for
         details.

     Defaults to true iff the OpenTelemetry Java API dependency [2] was
     present when Telemere's namespace was evaluated/compiled.

     Override with \"true\" or \"false\":
             JVM property: `taoensso.telemere.open-telemetry-mode`
             Env variable: `TAOENSSO_TELEMERE_open-telemetry-mode`
       Classpath resource: `taoensso.telemere.open-telemetry-mode`

     [1] Ref. <https://github.com/open-telemetry/opentelemetry-java>,
     [2] `io.opentelemetry/opentelemetry-api` on Maven"

     impl/enabled:otel-mode?))

#?(:clj
   (defn otel-get-default-providers
     "Experimental, subject to change. Feedback welcome!

     When `otel-mode?` is true, returns map with keys:
...
#?(:clj
   (def ^:dynamic *otel-tracer*
     ...
     See also `otel-mode?`, `otel-get-default-providers`."
     (enc/compile-when impl/present:otel?
       (delay
